...
function requireTier(...tiers: string[]) {
  return t.middleware(async (opts) => {
    const { ctx, next } = opts;
    if (!ctx.user) {
...
export const proQuery = authedQuery.use(requireTier("pro", "analyst"));
export const analystQuery = authedQuery.use(requireTier("analyst"));
